...
public:
    ...
    TDigest(std::vector<Bin> _bins = {}, int _delta = 10) : bins(_bins), delta(_delta) {
        if (!_bins.empty()) {
            bins = compress(_bins);
        }
        bins.reserve(delta);
...
private:
    ...
    std::vector<Bin> merge_bins(const std::vector<Bin>& xs, const std::vector<Bin>& ys) const {
